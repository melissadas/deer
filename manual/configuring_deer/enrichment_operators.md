...
The filter enrichment operator define the set of triple to be selected using either the single parameter `:sparqlConstrucyQuery` or `:selectors`.

The `:sparqlConstructQuery` takes precedence over `:selectors` and will accept a string literal that must be a valid SPARQL CONSTRUCT query,
...
On the other hand, the `:selectors` parameter accepts a list of selector types.
By selector we mean the triple patterns to be applied to the input dataset.
The filter enrichment operator has three basic selectors types. i.e. the `:subject`, `:predicate` and `:object` selectors that can be combined as required.
